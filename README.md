...
`loru` is a lightweight CLI to manage Loru projects (plugins, pages/tenants,
platform components).
...
Run from a clone of this repository (imports resolve from GitHub, no sibling
repos needed):
...
- If no config, per-entry metadata files (`plugin.toml` / `tenant.toml`) are
  still honored for schema version when validating.
...
loru dev fmt                # apply fmt + lint --fix where supported
...
- **Schema fetch/validate**: semver-aware fetcher (tags in `loru-schemas`),
  caches under `.loru/cache/schemas`. Validation uses Taplo against `loru.toml`.
- **Fmt**: Runs fmt/lint stages with fixes where supported (deno fmt, deno lint
  --fix, cargo fmt/clippy, taplo fmt/lint) across workspace members and targets.
- **Checks**: Runs fmt/lint defaults for detected Deno/Rust projects, then
  executes per-stage `[[check.task]]` hooks
  (precheck/fmt/lint/check/test/postcheck) across workspace members/targets.
- **Build**: Executes declared `[[build.task]]` phases and then builds detected
  Deno/Rust projects.
